...
With only ~18 measurements (12 working electrodes → 9 + 6 + 3 Wenner
...
structure that is not in the ground. The script uses `lam=20` to keep the
model smooth. A useful habit with data this sparse: treat any feature
that disappears when you double `lam` as an artefact.

## Status

The pipeline is written against sketch 06's output format but has not
been exercised on a real 2D field dataset — the instrument so far has one
