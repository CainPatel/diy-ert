...
out the chips: in binary the dead channels are 0000, 0111, 1011 and
1100, which share no common address bit, so no single stuck address line
can explain them. The survey firmware routes around dead channels with
the `CH[]` lookup table ([firmware/README.md](../README.md)).
