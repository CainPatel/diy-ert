...
serial output is the data file; the sketch prints nothing else. Capture
it verbatim as `survey.dat` and feed it to
[analysis/invert_pygimli.py](../../analysis/invert_pygimli.py).

**The `CH[]` table** maps logical electrode position (0-11, twelve
equally spaced rods along the line) to a working mux channel, so the
dead channels (0, 7, 11, 12, breadboard contact faults) are invisible to
the survey logic and the field geometry stays uniform. If your build has
different dead channels, run sketch 03 and edit the table.
...
working channels, but a 2D field survey has not been done yet. See the
[repository README](../../README.md) for status and
